...
// ForgotPasswordA
class ForgotPasswordA extends Component {
  constructor(props) {
    super(props);
  ...
  saveData = async () => {
    const data = await actions.resetPassword({
      email: this.state.email,
    });
    if (!data.errors) {
      utils.showAlert('Forgot Password', data.message);
    } else {
      utils.showAlert('Forgot Password', data.errors.email[0]);
    }
    this.setState({
      isLoading: false,
    });
...
const mapStateToProps = (state) => {
  return {
    resetPassword: state.login.data,
  };
};

const mapDispatchToProps = { loginFailure, loginSuccess };

export default connect(mapStateToProps, mapDispatchToProps)(ForgotPasswordA);
